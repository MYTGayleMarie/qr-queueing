...
function ReportExpense() {
  ...
  function generateInventory() {
    axios({
      ...
      .then((res) => {
        console.log(res);
        var monthly_inventory = res.data.data.inventory_expense_per_month.sort(
          (a, b) => a.item.localeCompare(b.item)
        );
        console.log(res.data.data.inventory_per_month);
        var monthly_inventory_price =
          res.data.data.inventory_expense_price_per_month.sort((a, b) =>
            a.item.localeCompare(b.item)
          );

        var monthly_inventory_adj = monthly_inventory.map((data) => {
          return {
            name: data.name,
            unit: data.unit,
        ...
        var monthly_inventory_price_adj = monthly_inventory_price.map(
          (data) => {
            return {
              name: data.name,
              unit: data.unit,
        ...
        const worksheet = XLSX.utils.json_to_sheet([
          {},
          ...monthly_inventory_adj,
        ]);
        const worksheet2 = XLSX.utils.json_to_sheet([
          {},
          ...monthly_inventory_price_adj,
        ]);
        ...
        XLSX.utils.book_append_sheet(
          workbook,
          worksheet,
          "Expense Item Inventory"
        );
        XLSX.utils.book_append_sheet(
          workbook,
          worksheet2,
          "Expense Item Cost Inventory"
        );
        XLSX.writeFile(
          workbook,
          "QRDiagnosticsMonthlyExpenseItemInventoryReport-" +
