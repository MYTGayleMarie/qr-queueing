  ...
  React.useEffect(() => {
    axios({
      ...
      params: {
        api_key: window.$api_key,
        token: userToken.replace(/['"]+/g, ""),
        hmo_code: discountInfo.discount_code,
        requester: userId,
        status: "unbilled",
      },
    })
      .then(function (response) {
        ...
        array.forEach(function (item, index) {
          ...
          if (existing.length) {
            ...
            output[existingIndex].customer = output[
              existingIndex
            ].customer.concat(item.customer)
            output[existingIndex].total_amount = output[
              existingIndex
            ].total_amount.concat(" " + item.total_amount)
          } else {
  ...
  return (
    <div>
      ...
      <div className="active-cont">
        <Header type="thick" title="ADD HMO INVOICE" />
        <ToastContainer />
        ...
        {info.length == 0 && (
          <div className="row d-flex justify-content-end">
            <button className="back-btn less-width" onClick={handleDelete}>
              DELETE DISCOUNT
            </button>
          </div>
        )}
