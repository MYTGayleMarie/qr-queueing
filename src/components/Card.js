...
function Card({totalData, todayData, services, packages, link, title, color, disable, excelReport=false, handler}) {
    return (
        <NavLink to={link} class="report-link">
            ...
            <div className='card-container' onClick={handler}>
                ...
                <div className='row'>
                    <div className='col-sm-3 d-flex justify-content-end'>
                        {totalData != "MD REPORTS" && totalData!= "REFERRALS" && totalData != "INVENTORY" && totalData != "ITEM INVENTORY" && totalData != "ITEM" && totalData != "EXPENSE" && totalData != "ANNUAL REPORT" &&(
            ...
            <div className='row'>
                <div className='col-sm-3 d-flex justify-content-end'>
                    {totalData != "MD REPORTS" && totalData!= "REFERRALS" && totalData != "INVENTORY" && totalData != "ITEM" && totalData != "EXPENSE" && totalData != "ANNUAL REPORT" && (
                        <img src={cashierIcon} alt={"report"} class="report-card icon"></img>
                    )}
                </div>
                <div className='col-sm-9 report-data'>
